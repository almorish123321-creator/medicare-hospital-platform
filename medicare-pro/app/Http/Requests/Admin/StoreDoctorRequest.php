...
class StoreDoctorRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'specialty' => ['required', 'string', 'min:2', 'max:255'],
            'department_id' => ['required', 'integer', 'exists:departments,id,hospital_id,' . $hospitalId],
     ...
     *
     * Ensures the subscription plan has available doctor slots.
     */
    public function withValidator(\Illuminate\Validation\Validator $validator): void
    {
        $validator->after(function (\Illuminate\Validation\Validator $validator) {
            ...
            // Check subscription plan doctor limit
            $hospital = \App\Models\Hospital::find($hospitalId);
            if ($hospital && $hospital->subscriptionPlan) {
                $currentDoctorCount = \App\Models\Doctor::whereHas('department', fn ($q) => $q->where('hospital_id', $hospitalId))
                    ->count();

                if ($hospital->subscriptionPlan->max_doctors && $currentDoctorCount >= $hospital->subscriptionPlan->max_doctors) {
                    $validator->errors()->add('user', __('validation.doctor_limit_reached'));
                }
            }
        });
    }
