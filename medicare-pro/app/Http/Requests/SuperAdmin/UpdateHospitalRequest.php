...
class UpdateHospitalRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'email' => ['sometimes', 'required', 'string', 'email', 'max:255', 'unique:hospitals,email,' . ($hospital ? $hospital->id : null)],
            'phone' => ['sometimes', 'required', 'string', 'max:50', 'regex:/^[+]?[\d\s\-\(\)]{7,50}$/'],
            'address' => ['sometimes', 'required', 'string', 'min:5', 'max:500'],
            'logo' => ['sometimes', 'nullable', 'image', 'mimes:jpeg,png,jpg,svg,webp', 'max:2048'],
            'subscription_plan_id' => ['sometimes', 'nullable', 'integer', 'exists:subscription_plans,id'],
            'admin_user' => ['sometimes', 'array'],
            'admin_user.name' => ['sometimes', 'required', 'string', 'min:2', 'max:255'],
            'admin_user.email' => ['sometimes', 'required', 'string', 'email', 'max:255'],
            'admin_user.phone' => ['sometimes', 'required', 'string', 'max:20', 'regex:/^[+]?[\d\s\-\(\)]{7,20}$/'],
            'admin_user.password' => ['sometimes', 'nullable', 'string', 'min:8', 'confirmed'],
            'admin_user.password_confirmation' => ['sometimes', 'nullable', 'string', 'min:8'],
        ];
    }
     ...
     *
     * Validates subscription plan and admin user email uniqueness.
     */
    public function withValidator(\Illuminate\Validation\Validator $validator): void
    {
        $validator->after(function (\Illuminate\Validation\Validator $validator) {
            ...
            $adminPhone = $this->input('admin_user.phone');

            if ($hospital) {
                // Get current admin user
                $currentAdmin = $hospital->users()->where('role', 'hospital_admin')->first();

                if ($adminEmail && $currentAdmin && $adminEmail !== $currentAdmin->email) {
                    $exists = \App\Models\User::where('email', $adminEmail)
                        ->where('id', '!=', $currentAdmin->id)
                        ->exists();

                    if ($exists) {
                        $validator->errors()->add('admin_user.email', __('validation.unique', ['attribute' => __('validation.attributes.email')]));
                    }
                }

                if ($adminPhone && $currentAdmin && $adminPhone !== $currentAdmin->phone) {
                    $exists = \App\Models\User::where('phone', $adminPhone)
                        ->where('id', '!=', $currentAdmin->id)
                        ->exists();

                    if ($exists) {
                        $validator->errors()->add('admin_user.phone', __('validation.unique', ['attribute' => __('validation.attributes.phone')]));
                    }
                }

                // Ensure hospital email and admin email are different
                $hospitalEmail = $this->input('email') ?? $hospital->email;
                if ($adminEmail && $hospitalEmail && $adminEmail === $hospitalEmail) {
